...
export const updateTransactionThunk = createAsyncThunk(
  ...
  async ({ id, transactionData }, thunkAPI) => {
    try {
      ...
      thunkAPI.dispatch(updateTransactionAction({ id, transaction: data.transaction }));
      return data.transaction;
    } catch (error) {
...
export const fetchTransactionsByCategory = createAsyncThunk(
  ...
  async (category, thunkAPI) => {
    try {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to fetch transactions by category';
      thunkAPI.dispatch(setError(errorMessage));
      return thunkAPI.rejectWithValue(errorMessage);
    } finally {
      thunkAPI.dispatch(setLoading(false));
